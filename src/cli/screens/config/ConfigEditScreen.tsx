...
export function ConfigEditScreen({ params }: ScreenProps): ReactElement {
    ...
    const handleSubmit = useCallback(async (values: FormValues) => {
        ...
        const connectionConfig = {
            ...
            port: isSqlite ? undefined : (values['port'] ? parseInt(String(values['port']), 10) : config.connection.port),
            database: String(values['database']),
            user: isSqlite ? undefined : (values['user'] ? String(values['user']) : undefined),
            // Only update password if provided
            password: isSqlite ? undefined : (values['password'] ? String(values['password']) : config.connection.password),
        }
    ...
    if (!configName) {
        ...
        return (
            <Panel title="Edit Configuration" paddingX={2} paddingY={1} borderColor="yellow">
                <Text color="yellow">No config name provided. Use: noorm config:edit &lt;name&gt;</Text>
            </Panel>
        )
    }
