 ...
 */
export function StatusList({ items, showIcons = true, maxVisible }: StatusListProps): ReactElement {
    ...
    return (
        <Box flexDirection="column">
            {hiddenCount > 0 && <Text dimColor>... {hiddenCount} more items above</Text>}
            {displayItems.map((item, index) => {
                ...
                return (
                    <Box key={key} gap={1}>
                        {showIcons && <Text color={config.color}>{config.icon}</Text>}
