/**
 * CLI Components.
 *
 * Shared UI components for the noorm TUI.
 *
 * @example
 * ```tsx
 * import { Panel, SelectList, Form, Confirm } from './components/index.js'
 * ```
 */

// Layout
export { Panel, Divider, MissingParamPanel, NotFoundPanel } from './layout/index.js';
export type { PanelProps, DividerProps, MissingParamPanelProps, NotFoundPanelProps } from './layout/index.js';

// Lists
export { SelectList, SearchableList, ActionList, StatusList } from './lists/index.js';
export type {
    SelectListProps,
    SelectListItem,
    SelectListRowState,
    SearchableListProps,
    SearchableListFilterState,
    ActionListProps,
    ActionItem,
    StatusListProps,
    StatusListItem,
    StatusType,
} from './lists/index.js';

// Forms
export { Form, TextInput } from './forms/index.js';
export type {
    FormProps,
    FormField,
    FormFieldType,
    FormValues,
    FormErrors,
    SelectOption,
} from './forms/index.js';

// Feedback (re-exports from @inkjs/ui + custom toast)
export { Spinner, ProgressBar, Alert, StatusMessage, Badge } from './feedback/index.js';
export { ToastProvider, ToastRenderer, useToast } from './feedback/index.js';
export type { Toast, ToastVariant } from './feedback/index.js';

// Dialogs
export { Confirm, ProtectedConfirm, SmartConfirm, FilePicker, KeyHandler } from './dialogs/index.js';
export type {
    ConfirmProps,
    ProtectedConfirmProps,
    SmartConfirmProps,
    FilePickerProps,
    FilePickerMode,
    KeyHandlerProps,
} from './dialogs/index.js';

// Status
export { LockStatus, ConnectionStatus, isDatabaseNotFoundError, StatementProgress } from './status/index.js';
export type {
    LockStatusProps,
    LockStatusType,
    ConnectionStatusProps,
    ConnectionStatusType,
    StatementProgressProps,
} from './status/index.js';

// Secrets
export {
    SecretDefinitionForm,
    SecretDefinitionList,
    SecretDefinitionListHelp,
    SecretValueForm,
    SecretValueList,
    SecretValueListHelp,
    SECRET_TYPE_OPTIONS,
    validateSecretKey,
    checkDuplicateKey,
} from './secrets/index.js';
export type {
    SecretDefinitionFormProps,
    SecretDefinitionListProps,
    SecretValueFormProps,
    SecretValueListProps,
    SecretValueItem,
    SecretValueSummary,
    StageSecret,
    SecretType,
} from './secrets/index.js';

// Overlays
export { LogViewerOverlay, TextOverlay } from './overlays/index.js';
export type { LogViewerOverlayProps, TextOverlayProps } from './overlays/index.js';

// Terminal (SQL REPL)
export { SqlInput, ResultTable } from './terminal/index.js';
export type { SqlInputProps, ResultTableProps } from './terminal/index.js';
