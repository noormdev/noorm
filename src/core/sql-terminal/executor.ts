...
import { SERVER_CANCEL, SESSION_ID_SQL, hasServerSideCancel, readSessionId } from '../connection/session.js';
import type { ServerCancel } from '../connection/session.js';
import type { Dialect } from '../connection/types.js';
import type { SqlExecutionResult } from './types.js';

export { hasServerSideCancel };
 ...
 *
 * Reads the shared cancel table so the UI cannot drift into claiming a
...
async function runWithServerCancel(
    ...
    return db.connection().execute(async (pinned) => {
        ...
        const probe = await sql.raw<{ id?: unknown }>(sessionIdSql).execute(pinned);
        ...
        const onAbort = () => {
            ...
            void attempt(() => cancel(db, sessionId));
...
function runQuery(
    ...
    const sessionIdSql = dialect ? SESSION_ID_SQL[dialect] : undefined;
    const cancel = dialect ? SERVER_CANCEL[dialect] : undefined;

    if (!sessionIdSql || !cancel) {

        return raceAbort(sql.raw(query).execute(db), signal);

    }

    return raceAbort(runWithServerCancel(db, query, sessionIdSql, cancel, signal, arming), signal);
