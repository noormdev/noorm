...
import type { Changeset, ChangesetFile, ChangesetFileType } from './types.js';
import {
 ...
 * ```
 */
export async function parseChangeset(folderPath: string, schemaDir?: string): Promise<Changeset> {
    ...
    const [changeFiles, changeErr] = await attempt(() => scanFolder(changePath, schemaDir));
    ...
    const [revertFiles, revertErr] = await attempt(() => scanFolder(revertPath, schemaDir));
...
export async function discoverChangesets(
    ...
    const [entries, readErr] = await attempt(() => readdir(changesetsDir, { withFileTypes: true }));
    ...
    for (const folder of folders) {
        ...
        const [changeset, parseErr] = await attempt(() => parseChangeset(folderPath, schemaDir));
 ...
 * ```
 */
export async function resolveManifest(manifestPath: string, schemaDir: string): Promise<string[]> {
 ...
 */
async function scanFolder(folderPath: string, schemaDir?: string): Promise<ChangesetFile[]> {
