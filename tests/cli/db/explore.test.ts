/**
 * cli: noorm db explore — result on stdout, diagnostics on stderr (CP4).
 *
 * Mirrors tests/cli/change/list.test.ts's harness. Before the CP4 fix, the
 * "Database Overview" summary went to `logger.info`, which always routed
 * to stderr in text mode — invisible to a CI step capturing stdout only.
 */
...
describe('cli: noorm db explore — output streams', () => {
    ...
    beforeEach(() => {
        ...
        tmpDir = mkdtempSync(join(tmpdir(), 'noorm-db-explore-'));
        fakeHome = mkdtempSync(join(tmpdir(), 'noorm-db-explore-home-'));
    ...
    function runExplore(args: string[] = []) {

        return spawnSync('node', [CLI, 'db', 'explore', ...args], {
            cwd: tmpDir,
            encoding: 'utf-8',
            env: identityEnv,
        });
    ...
    it('prints the overview summary to stdout, not stderr', async () => {

        await seedConfig();

        const result = runExplore();

        expect(result.status).toBe(0);
        expect(result.stdout).toContain('Database Overview');
