-- MSSQL Views for Test Schema
-- Adapted from PostgreSQL: FILTER clause → CASE WHEN with SUM

CREATE OR ALTER VIEW v_active_users AS
SELECT
    id,
    email,
    username,
...
CREATE OR ALTER VIEW v_todo_lists_with_counts AS
SELECT
    tl.id,
    tl.user_id,
    tl.title,
...
GROUP BY tl.id, tl.user_id, tl.title, tl.description, tl.color, tl.position, tl.created_at, tl.updated_at;
GO

CREATE OR ALTER VIEW v_active_todo_items AS
SELECT
    ti.id,
    ti.list_id,
    ti.title,
