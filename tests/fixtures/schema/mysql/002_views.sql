-- MySQL Views for Test Schema
-- Adapted from PostgreSQL: FILTER clause → CASE WHEN
...
SELECT
    ...
    SUM(CASE WHEN ti.deleted_at IS NULL AND ti.is_completed = TRUE THEN 1 ELSE 0 END) AS completed_items,
    SUM(CASE WHEN ti.deleted_at IS NULL AND ti.is_completed = FALSE THEN 1 ELSE 0 END) AS pending_items
FROM todo_lists tl
