...
A [[tags]] belonging to a [[posts]] row. Its key is the basetype's key, and `target_type` is pinned so the pair can only attach to a row the basetype already marked `POST`.
