...
columns:
  ...
  subject_type:
    type: text
    desc: "Discriminator, drawn from the photo_subject_types table rather than a CHECK list. Joins a unique key so it can travel into each subtype's foreign key."
  created_at:
    type: datetime
examples:
  - photo_id: 1
