...
| paths | PathConfig | Yes | File system paths for schema and changesets |
...
| changesets | string | `changesets` | Path to changeset directories |
...
| paths | object | Schema and changeset directories |
| isTest | boolean | Test database flag |
...
Changesets live on disk as directories with a specific structure.
...
When read from disk, changesets are parsed into:
...
| changeFiles | ChangesetFile[] | Files in `change/` subdirectory |
| revertFiles | ChangesetFile[] | Files in `revert/` subdirectory |
...
Changeset folder names follow a convention:
...
Used in `__noorm_changeset__` and changeset results.
...
| changeset failed | Parent changeset failed |
...
The changeset file system provides versioned migrations, while runtime types enable flexible execution modes (dry run, preview, force).
