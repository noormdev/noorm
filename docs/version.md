...
export const v2: SchemaMigration = {
    version: 2,
    description: 'Add tags column to changeset table',

    async up(db: Kysely<unknown>): Promise<void> {
        await db.schema
            .alterTable('__noorm_changeset__')
...
await sql`ALTER TABLE __noorm_changeset__ ADD COLUMN metadata JSONB DEFAULT '{}'`.execute(db)
```
...
await db.selectFrom(NOORM_TABLES.changeset).where('status', '=', 'success').execute()
```
...
| `schema_version` | Database tracking tables version |
...
// Type-safe inserts
const newChangeset: NewNoormChangeset = {
    name: 'add-users-table',
    change_type: 'changeset',
    direction: 'change',
    status: 'pending',
}
await db.insertInto('__noorm_changeset__').values(newChangeset).execute()
```
