...
The revert scripts execute in reverse sequence order. If your change had `001_create-table.sql` and `002_add-indexes.sql`, the revert runs `002_drop-indexes.sql` first, then `001_drop-table.sql`.
...
Rewind walks backward through your applied changes, reverting each one. It respects the reverse chronological order, undoing newer changes before older ones.
...
3. **Executes in reverse order** - Files run from highest sequence number to lowest.
...
The history shows which specific file failed and includes the error message from the database. See [History](/guide/migrations/history) for detailed debugging workflows.
...
noorm -H change revert 2024-02-15-broken-migration
```
...
- [History](/guide/migrations/history) - Track and debug execution history
- [Changes](/guide/migrations/changes) - Change structure and file types
