-- Recreate vw_Tag to surface the new `color` column on Tag.
-- DROP+CREATE (not CREATE OR REPLACE) because the column shape changes.
-- `t.color` is threaded into all 5 UNION ALL branches immediately after
-- `t.provenance_id` to keep the column ordering stable across branches.
